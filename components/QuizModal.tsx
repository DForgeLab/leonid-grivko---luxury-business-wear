      ...
      <div id="quiz-modal-content" className="bg-backgroundLight text-textOnLight rounded-2xl shadow-2xl w-full max-w-2xl relative p-6 sm:p-8 md:p-12 transform transition-all animate-fade-in-up" onClick={e => e.stopPropagation()}>
        ...
        {submitted ? (
          <div className="text-center py-12">
            <h2 className="text-2xl sm:text-3xl font-bold text-primary mb-4 font-serif">Спасибо!</h2>
            <p className="text-base sm:text-lg text-textSecondaryOnLight mb-8">Ваша заявка принята. Наш стилист скоро свяжется с вами для обсуждения персональной подборки.</p>
          ...
          <div>
            ...
            {step < totalQuestions && (
              <div className="animate-fade-in">
                <h2 className="text-2xl sm:text-3xl font-bold text-center mb-10 font-serif">{questions[step].question}</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {questions[step].options.map(option => (
                    <button 
                      key={option} 
                      onClick={() => handleAnswerSelect(questions[step].id, option)}
                      className={`w-full text-left p-4 border rounded-lg focus:outline-none transition-all duration-200 font-semibold text-base ${selectedOption === option ? 'bg-primary text-white border-primary' : 'border-black/20 bg-surface hover:border-primary'}`}
                    >
                      {option}
                    </button>
                  ))}
            ...
            {step === totalQuestions && (
              <div className="animate-fade-in">
                <h2 className="text-2xl sm:text-3xl font-bold text-center mb-4 font-serif">Получите вашу персональную подборку</h2>
                <p className="text-center text-textSecondaryOnLight mb-8 text-base">Оставьте ваши контактные данные, и наш стилист свяжется с вами.</p>
                <form onSubmit={handleSubmit} className="space-y-5">
                   <div>
                       <label htmlFor="name" className="block text-sm font-medium text-textSecondaryOnLight mb-1">Ваше имя</label>
                       <input type="text" name="name" id="name" required className="w-full px-4 py-3 border border-black/20 rounded-lg focus:ring-2 focus:ring-primary focus:border-primary text-base bg-surface placeholder:text-textSecondaryOnLight/50" placeholder="Анастасия" />
                   </div>
                   <div>
                       <label htmlFor="phone" className="block text-sm font-medium text-textSecondaryOnLight mb-1">Телефон</label>
                       <input type="tel" name="phone" id="phone" required className="w-full px-4 py-3 border border-black/20 rounded-lg focus:ring-2 focus:ring-primary focus:border-primary text-base bg-surface placeholder:text-textSecondaryOnLight/50" placeholder="+7 (999) 123-45-67" />
                   </div>
            ...
            <div className="mt-12 pt-6 border-t border-black/10 flex justify-between items-center h-8">
              {step > 0 && step <= totalQuestions && (
                <button onClick={handleBack} className="text-sm font-semibold text-textSecondaryOnLight hover:text-primary transition-colors flex items-center group">
